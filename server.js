...
const rooms = {};
// Player scores storage (persistent across games)
const playerScores = {};
...
function handlePostPlayerChange(roomId) {
  const room = rooms[roomId];
  if (!room) return;
  ...
  if (room.started) {
    ...
    if (alivePlayers.length <= 1) {
      const winners = alivePlayers.map((w) => ({ id: w.id, name: w.name }));

      // Award 5 points to winners
      winners.forEach((winner) => {
        if (!playerScores[winner.id]) {
          playerScores[winner.id] = {
            name: winner.name,
            score: 0,
            roundsWon: 0,
          };
        }
        playerScores[winner.id].score += 5;
        playerScores[winner.id].roundsWon += 1;
      });

      // Increment round count (牌局紀錄：每次出現贏家時 +1)
      ...
      io.to(roomId).emit("game_over", {
        ...
        playerStats: room.players.map((p) => ({
          id: p.id,
          name: p.name,
          score: playerScores[p.id]?.score || 0,
          roundsWon: playerScores[p.id]?.roundsWon || 0,
        })),
      });
      room.started = false;
    }
...
function broadcastRoomList() {
  const roomList = Object.values(rooms)
    .filter(
      (room) =>
        !room.started && room.players.length > 0 && room.players.length < 4
    )
    .map((room) => ({
      id: room.id,
  ...
  console.log(
    `Server: Broadcasting room list with ${roomList.length} rooms:`,
    roomList.map((r) => r.id)
  );
  // 發送給所有客戶端（包括在 join game 頁面的客戶端）
  io.emit("rooms_list", { rooms: roomList });
}
...
function removePlayerFromRoom(roomId, socket) {
  ...
  if (idx !== -1) {
    ...
    // If host left and there are remaining players, transfer host to the first remaining player
  ...
  for (let m = 0; m < multi; m++) {
    for (const s of suits) {
      for (const r of ranks) {
        deck.push({
          id: `${s}-${r}-${m}-${Math.random().toString(36).slice(2, 8)}`,
          suit: s,
          rank: r,
        });
      }
...
function nextAliveIndex(room, startIndex, step = 1) {
  ...
  for (let attempts = 0; attempts < players.length * 2; attempts++) {
    // Iterate more than once around the table to be safe
    currentIdx =
      (currentIdx + room.direction + players.length) % players.length; // Move to the next player in direction
...
function applyCardEffect(room, playerId, card, optionalTarget) {
  ...
  if (r === "A") {
    ...
    console.log(
      `Server: Player ${playerName} played J. skipNext is now false.`
    );
  } else if (r === "4") {
    room.direction *= -1;
...
io.on("connection", (socket) => {
  socket.on("create_room", ({ name }) => {
    if (!name) {
      return socket.emit("error_message", "name required");
    }

    if (socket.data.roomId) {
      removePlayerFromRoom(socket.data.roomId, socket);
    }
    ...
    do {
      ...
      attempts += 1;
    } while (rooms[roomId] && attempts < 1000);

    if (rooms[roomId]) {
      return socket.emit("error_message", "unable to generate roomId");
    }
    ...
    if (!playerScores[socket.id]) {
      playerScores[socket.id] = { name, score: 0, roundsWon: 0 };
    } else {
      playerScores[socket.id].name = name; // Update name if changed
    }
  ...
  socket.on("join_room", ({ roomId, name }) => {
    ...
    if (player) {
      ...
      player = {
        id: socket.id,
        name: name || `P${room.players.length + 1}`,
        eliminated: false,
        hand: [],
    ...
    // Initialize player score if not exists
    if (!playerScores[socket.id]) {
      playerScores[socket.id] = { name: player.name, score: 0, roundsWon: 0 };
    } else {
      playerScores[socket.id].name = player.name; // Update name if changed
    }
  ...
  socket.on("start_game", ({ roomId }) => {
    ...
    if (!player || !player.isHost) {
      console.log("Server: Only host can start the game");
    ...
    if (room.players.length < 2) {
      console.log(
        "Server: Not enough players in room",
        roomId,
        ". Emitting error_message."
      );
      return socket.emit(
        "error_message",
        "Not enough players to start the game (minimum 2 required)."
      );
    }
    ...
    io.to(roomId).emit(
      "log",
      `Game started. currentValue=${room.currentValue}`
    );
    io.to(roomId).emit("turn_to", {
  ...
  socket.on("play_card", ({ roomId, cardId, targetId }) => {
    ...
    if (!room || !room.started)
      return socket.emit("error_message", "game not started");
    ...
    // advance turn
    console.log(
      `Server: Before turn advancement. Current turnIndex: ${room.turnIndex}, Direction: ${room.direction}, skipNext: ${res.skipNext}, setNextPlayer: ${res.setNextPlayer}`
    );

    let next = -1;
    if (res.setNextPlayer) {
      const targetPlayerIndex = room.players.findIndex(
        (p) => p.id === res.setNextPlayer
      );
      if (
        targetPlayerIndex !== -1 &&
        !room.players[targetPlayerIndex].eliminated
      ) {
    ...
    if (next === -1) {
      // if target not set or invalid, proceed as normal
      let step = 1;
      if (res.skipNext) {
        step = step * 2;
    ...
    console.log(
      `Server: After turn calculation, next turnIndex will be: ${next}`
    );
    ...
    room.turnIndex = next;
    console.log(
      `Server: Turn advanced to player at index: ${room.turnIndex}, ID: ${
  ...
  socket.on("leave_room", ({ roomId }) => {
    removePlayerFromRoom(roomId, socket);
  });

  socket.on("concede", ({ roomId }) => {
    const room = rooms[roomId];
    if (!room || !room.started) {
      return socket.emit("error_message", "game not started");
    }
    const player = room.players.find((p) => p.id === socket.id);
    if (!player) {
      return socket.emit("error_message", "player not found");
    }
    if (player.eliminated) {
      return socket.emit("error_message", "player already eliminated");
    }
    player.eliminated = true;
    io.to(roomId).emit("player_eliminated", {
      playerId: player.id,
    ...
    io.to(roomId).emit("log", `${player.name} conceded`);
  ...
  socket.on("request_rooms", () => {
    ...
    console.log(
      `Server: Client requested rooms, sending ${roomList.length} rooms:`,
      roomList.map((r) => r.id)
    );
